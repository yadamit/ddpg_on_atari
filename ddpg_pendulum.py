...
# Critic Network
class Critic:
	def __init__(self, input_layer=4, hidden_layer=100, output_layer=1):
	...
	def update_target_network(self, tau):
		...
		sess.run([self.a,self.b,self.c,self.d])
...
# Actor Network
class Actor:
	def __init__(self, critic, input_layer=3, hidden_layer=100, output_layer=1, action_range=2.0):
		self.tau = 0.001
		self.input_layer = input_layer
		self.hidden_layer = hidden_layer
		self.output_layer = output_layer
		...
		self.raw_actor_action = tf.nn.tanh(tf.add(tf.matmul(self.actor_h1, self.actor_W2), self.actor_b2))
		...
		self.target_h1 = tf.nn.relu(tf.add(tf.matmul(self.target_input, self.target_W1), self.target_b1))
		self.raw_target_action = tf.nn.tanh(tf.add(tf.matmul(self.target_h1,self.target_W2), self.target_b2))
		...
		self.actor_optimizer = tf.train.AdamOptimizer(0.0001).minimize(-1*self.J, var_list=[self.actor_W1, self.actor_b1, self.actor_W2, self.actor_b2])
...
critic = Critic(state_dim+action_dim,1000,1)
actor = Actor(critic,state_dim,1000,1, action_range)
...
####################### Testing ####################################
def Test(actor, env, num_episodes=1000, num_time_steps=300):
	# num_episodes = 1000
	# num_time_steps = 300
	for epoch in range(num_episodes):
		print(epoch+1, end='  ')
